...
val appModule = module {
    ...
    single<VideoJournalRepository> { VideoJournalRepositoryImpl(get()) }
    single<VideoStorageRepository> { FileVideoStorageRepository(get()) }

    factory { ObserveVideoEntriesUseCase(get()) }
    factory { CreatePendingCaptureUriUseCase(get()) }
    factory { SaveCapturedVideoUseCase(get(), get()) }

    viewModel { VideoJournalViewModel(get(), get(), get()) }
}
